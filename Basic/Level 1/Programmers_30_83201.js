...
function solution(scores) {
  // 자기 자신이 준 점수가 배열에서 가장 작거나 큰 수면 제외
  let result = "";
  let add = [];
  let students = scores.length;
  let avg;
  ...
  for (let i = 0; i < scores.length; i++) {
    ...
    if (add[0] === scores[i][i]) {
      if (add.lastIndexOf(scores[i][i]) === 0) {
        add.splice(0, 1);
        students = scores.length - 1;
      }
    } else if (add[add.length - 1] === scores[i][i]) {
      if (add.indexOf(scores[i][i]) === add.length - 1) {
