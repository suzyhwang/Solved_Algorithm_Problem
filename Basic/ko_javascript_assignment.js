...
document.body.childNodes[1] // 첫 번째 노드는 공백이므로 두 번째 노드를 가져옴
// <ul> DOM 노드
...
// * elem.lastChild.nextSibling은 항상 null 일까요?
// * elem.children[0].previousSibling은 항상 null 일까요?
// 1. elem.lastChild는 항상 마지막 노드이기 때문에 nextSibling이 없습니다.
// 2. 아닙니다. elem.children[0]은 요소 노드 중 첫 번째 자식 노드를 나타내기 때문입니다. 
// 이 앞에 요소 노드가 아닌 다른 노드가 올 수도 있습니다. 
// previousSibling은 텍스트 노드가 될 수도 있습니다.

// ! 주의 사항 : 두 경우 모두 자식 노드가 없는 경우 에러가 발생합니다.
// 자식 노드가 없으면 elem.lastChild은 null이 되기 때문에 elem.lastChild.nexSibling에
// 접근할 수 없습니다. 그리고 컬렉션 elem.children은 빈 배열 [] 같이 빈 상태가 됩니다.

// 요소 검색하기
// 테이블과 폼이 있는 문서가 있다고 가정합시다.

// 후손 노드 개수 세기
// ul과 li노드로 구성된 트리 구조 문서가 있다고 가정해봅시다.
